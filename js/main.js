...
const targetLanguageSelect = document.getElementById("header-target-language")
...
targetLanguageSelect.value = CONFIG.TRANSLATION?.TARGET_LANGUAGE || "auto"
...
const CONFIG_PRESETS = {
  translator: {
    voice: "Aoede",
    sampleRate: 22000,
    systemInstruction: "TRANSLATE ONLY. NO TALKING. NO CONVERSATION. NO QUESTIONS. NO ANSWERS. JUST TRANSLATE. DO NOT DEVIATE. TRANSLATE.",
  },
}
...
async function updateConfiguration() {
  ...
  CONFIG.TRANSLATION.TARGET_LANGUAGE = newTargetLanguage
  ...
  localStorage.setItem("gemini_target_language", newTargetLanguage)
...
if (localStorage.getItem("gemini_target_language")) {
  CONFIG.TRANSLATION.TARGET_LANGUAGE = localStorage.getItem("gemini_target_language")
  targetLanguageSelect.value = CONFIG.TRANSLATION.TARGET_LANGUAGE
}
...
targetLanguageSelect.addEventListener("change", async () => {
  CONFIG.TRANSLATION.TARGET_LANGUAGE = targetLanguageSelect.value
  localStorage.setItem("gemini_target_language", targetLanguageSelect.value)
  
  if (isConnected) {
    logMessage("Reconnecting to apply language change...", "system")
    await disconnectFromWebsocket()
    await connectToWebsocket()
  }
  
  logMessage(`Target language changed to: ${targetLanguageSelect.options[targetLanguageSelect.selectedIndex].text}`, "system")
})
...
async function connectToWebsocket() {
  ...
  try {
    ...
    if (window.eburonAIServer) {
      window.eburonAIServer.updateConnectionStatus(true);
    }
...
function disconnectFromWebsocket() {
  ...
  // Update API server connection status
  if (window.eburonAIServer) {
    window.eburonAIServer.updateConnectionStatus(false);
  }
...
async function handleSendMessage() {
  ...
  // Track translation in API server
  if (window.translationAPIServer) {
    window.translationAPIServer.addTranslation(
      message,
      '', // Will be filled when assistant responds
      'auto', // Source language (auto-detected)
...
client.on("content", (data) => {
  if (data.modelTurn) {
    ...
    if (text) {
      ...
      // Update the last translation record with the assistant's response
      if (window.translationAPIServer && lastUserMessage) {
        const history = window.translationAPIServer.getTranslationHistory(1);
        if (history.length > 0 && !history[0].output) {
          // Update the most recent translation with the output
...
async function initializeHTTPServer() {
  try {
    // Simple HTTP server using fetch API to handle requests
    if (typeof window !== 'undefined') {
      // Override fetch to handle API requests
...
async function handleAPIRequest(url, options) {
  try {
    ...
    switch (url) {
      case '/api/server/start':
        if (options.method === 'POST') {
          ...
          result = window.eburonAIServer.startServer(body.autoConnect, body.config);
        }
        break;
      case '/api/server/stop':
        if (options.method === 'POST') {
          result = window.eburonAIServer.stopServer();
        }
        break;
      case '/api/server/status':
        result = window.eburonAIServer.getServerStatus();
        break;
      case '/api/connection/connect':
        if (options.method === 'POST') {
          const body = JSON.parse(options.body);
          result = window.eburonAIServer.connect(body.config);
        }
        break;
      case '/api/connection/disconnect':
        if (options.method === 'POST') {
          result = window.eburonAIServer.disconnect();
        }
        break;
      case '/api/connection/status':
        result = window.eburonAIServer.getConnectionStatus();
        break;
      case '/api/health':
        result = {
          status: 'healthy',
          ...
          uptime: '0s',
          apiStatus: {
            gemini: window.eburonAIServer.connectionData.isConnected ? 'connected' : 'disconnected',
            websocket: window.eburonAIServer.connectionData.isConnected ? 'active' : 'inactive'
          }
        };
        break;
      case '/api/audio/microphone/start':
        if (options.method === 'POST') {
          if (typeof startRecording === 'function') {
            startRecording();
            result = { success: true, isRecording: true, message: 'Microphone started successfully' };
          } else {
            result = { success: false, isRecording: false, message: 'Microphone not available' };
          }
        }
        break;
      case '/api/audio/microphone/stop':
        if (options.method === 'POST') {
          if (typeof stopRecording === 'function') {
            stopRecording();
            result = { success: true, isRecording: false, message: 'Microphone stopped successfully' };
          } else {
            result = { success: false, isRecording: false, message: 'Microphone not available' };
          }
        }
        break;
      case '/api/video/camera/start':
        if (options.method === 'POST') {
          if (typeof startVideo === 'function') {
            startVideo();
            result = { success: true, isActive: true, source: 'camera', message: 'Camera started successfully' };
          } else {
            result = { success: false, isActive: false, source: 'camera', message: 'Camera not available' };
          }
        }
        break;
      case '/api/video/camera/stop':
        if (options.method === 'POST') {
          if (typeof stopVideo === 'function') {
            stopVideo();
            result = { success: true, isActive: false, source: 'camera', message: 'Camera stopped successfully' };
          } else {
            result = { success: false, isActive: false, source: 'camera', message: 'Camera not available' };
          }
        }
        break;
      case '/api/video/screen/start':
        if (options.method === 'POST') {
          if (typeof handleScreenShare === 'function') {
            handleScreenShare();
            result = { success: true, isActive: true, source: 'screen', message: 'Screen sharing started successfully' };
          } else {
            result = { success: false, isActive: false, source: 'screen', message: 'Screen sharing not available' };
          }
        }
        break;
      case '/api/video/screen/stop':
        if (options.method === 'POST') {
          if (typeof stopScreenSharing === 'function') {
            stopScreenSharing();
            result = { success: true, isActive: false, source: 'screen', message: 'Screen sharing stopped successfully' };
          } else {
            result = { success: false, isActive: false, source: 'screen', message: 'Screen sharing not available' };
          }
        }
        break;
      case '/api/conversation/message':
        if (options.method === 'POST') {
          const body = JSON.parse(options.body);
          if (typeof sendMessage === 'function' && messageInput) {
            messageInput.value = body.message;
            sendMessage();
            result = { success: true, message: 'Message sent successfully' };
          } else {
            result = { success: false, message: 'Message sending not available' };
          }
        }
        break;
      case '/api/config/language':
        if (options.method === 'PUT') {
          const body = JSON.parse(options.body);
          if (typeof CONFIG !== 'undefined' && CONFIG.TRANSLATION) {
            CONFIG.TRANSLATION.TARGET_LANGUAGE = body.language;
            if (headerTargetLanguage) {
              headerTargetLanguage.value = body.language;
            }
            result = { success: true, updated: 'language', newValue: body.language, message: 'Language updated successfully' };
          } else {
            result = { success: false, message: 'Language configuration not available' };
          }
        }
        break;
      case '/api/config/voice':
        if (options.method === 'PUT') {
          const body = JSON.parse(options.body);
          if (typeof CONFIG !== 'undefined' && CONFIG.VOICE) {
            CONFIG.VOICE.NAME = body.voice;
            if (voiceSelect) {
              voiceSelect.value = body.voice;
            }
            result = { success: true, updated: 'voice', newValue: body.voice, message: 'Voice updated successfully' };
          } else {
            result = { success: false, message: 'Voice configuration not available' };
          }
        }
        break;
      case '/api/config/prompt':
        if (options.method === 'PUT') {
          const body = JSON.parse(options.body);
          if (typeof CONFIG !== 'undefined' && CONFIG.SYSTEM_INSTRUCTION) {
            CONFIG.SYSTEM_INSTRUCTION.TEXT = body.prompt;
            result = { success: true, updated: 'prompt', newValue: body.prompt, message: 'Prompt updated successfully' };
          } else {
            result = { success: false, message: 'Prompt configuration not available' };
          }
        }
        break;
      default:
    ...
    response.json = async () => ({ data: result });
    return response;
